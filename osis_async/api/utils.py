...
def proxy_view(view_cls):
    ...
    if not getattr(settings, 'OSIS_ASYNC_BASE_URL', False):
        # Do not proxy if OSIS_ASYNC_BASE_URL is not set
        return view_cls.as_view()

    def wrapped(request, *args, **kwargs):
        local_base_url = resolve_url('osis_async:task-list')
        url_for_remote_api = request.path
        url = settings.OSIS_ASYNC_BASE_URL + url_for_remote_api.replace(local_base_url, '')
        headers = {
...
class CorsAllowOriginMixin(APIView):
    ...
    def origin_found_in_white_lists(self, url):
        origins = [urlparse(o) for o in settings.OSIS_ASYNC_DOMAIN_LIST]
        return any(
